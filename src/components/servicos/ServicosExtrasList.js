...
import { Trash2, Edit, FileText, FileSpreadsheet, FileDown, Printer } from "lucide-react";
      ...
      >
        <table className="w-full text-sm md:text-base">
          ...
          <tbody>
            {data.length > 0 ? (
              data.map((srv) => (
                <tr key={srv.id} className="border-t hover:bg-slate-50 transition">

                  <td className="p-3 font-semibold">{srv.nome}</td>
