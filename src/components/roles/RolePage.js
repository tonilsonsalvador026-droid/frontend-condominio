...
const RolePage = () => {
  ...
  const rolesFiltradas = roles.filter((role) =>
    role.nome?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Deseja eliminar esta função?")) return;

    try {
      await api.delete(`/roles/${id}`);

      toast.success("Função eliminada com sucesso!");

      fetchRoles();
    } catch (err) {
      console.error(err);
      toast.error("Erro ao eliminar função.");
    }
  };
        ...
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6">
          ...
          <div>
            ...
            <p className="text-slate-600 mt-2">
              Gestão completa de papéis e funções do sistema.
            </p>

            {!mostrarForm && (
              <p className="text-sm text-slate-500 mt-3 font-medium">
                {rolesFiltradas.length} registos encontrados
              </p>
            )}
          </div>
