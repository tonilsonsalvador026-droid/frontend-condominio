  ...
  return (
    <div className="space-y-8 w-full">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            ...
            <div className="relative flex-1 lg:w-96">
              ...
              <input
                ...
                className="w-full pl-12 pr-6 py-4 bg-white border rounded-2xl shadow-lg"
              />
            </div>
      ...
      <div className="flex justify-center gap-3">
        ...
        <button onClick={() => setPaginaAtual((p) => Math.min(p + 1, totalPaginas))}>
          Próxima
        </button>
      </div>

      {/* EXPORTS (FINAL COM ANIMAÇÃO) */}
      {filteredRecibos.length > 0 && (
        <div className="bg-white/60 backdrop-blur-xl rounded-3xl p-6 border shadow-xl">
          <div className="flex flex-wrap gap-3 justify-center">

            <button onClick={exportCSV} className="group px-6 py-3 bg-blue-600 text-white font-bold rounded-2xl shadow-lg hover:-translate-y-1 transition-all flex items-center gap-2">
              <FileText className="w-4 h-4" /> CSV
            </button>

            <button onClick={exportExcel} className="group px-6 py-3 bg-emerald-600 text-white font-bold rounded-2xl shadow-lg hover:-translate-y-1 transition-all flex items-center gap-2">
              <FileSpreadsheet className="w-4 h-4" /> Excel
            </button>

            <button onClick={exportPDF} className="group px-6 py-3 bg-red-600 text-white font-bold rounded-2xl shadow-lg hover:-translate-y-1 transition-all flex items-center gap-2">
              <FileDown className="w-4 h-4" /> PDF
            </button>

            <button onClick={handlePrint} className="group px-6 py-3 bg-slate-600 text-white font-bold rounded-2xl shadow-lg hover:-translate-y-1 transition-all flex items-center gap-2">
